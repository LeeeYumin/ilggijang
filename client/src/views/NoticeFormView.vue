  ...
  export default {
    ...
    created() {
      ...
      if (this.searchNo != null && this.searchNo != undefined) {
        ...
        this.noticeInfo.join_date = this.getDate('');
      }
    },
    methods: {
      async getNoticeInfo() {
        let result = await axios.get('/api/admin/notice/' + this.searchNo)
          ...
          .then(result => {
            ...
            if (count == 0) {
              ...
              if (result.data.insertId > 0) { // 등록일 경우 추가 작업
                this.noticeInfo.notice_no = result.data.insertId;
              }
            }
            console.log(result);
          })
        ...
        if (notice_no == null) {  // 등록
          ...
          data = {
            "param": {
                ...
                fix_end_date: this.noticeInfofix_end_date
           }
         }
