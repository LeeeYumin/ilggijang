...
.book_info{position:relative; padding:20px;}
        ...
        async cartInsert() {
            if(this.selected.length <= 0){
                ...
                if( result ) {
                    ...
                    let saveItem = [];
                    for(let i = 0; i < this.saveList.length; i++){
                        if(this.selected.includes(this.saveList[i].prdt_no)){
                            saveItem.push({ 
                                                book_no : this.saveList[i].prdt_no,
                                                book_name : this.saveList[i].book_name,
                                                quantity : this.saveList[i].quantity,
                                                book_img : this.saveList[i].book_img,
                                                book_price : this.saveList[i].book_price,
                                                total_price : this.saveList[i].total_price
                                            });
                            sessionStorage.setItem("saveItem", JSON.stringify(saveItem));
                        }
                    }
